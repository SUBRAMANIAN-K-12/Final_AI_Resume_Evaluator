...
import { jobAnalysis } from "../../../../service/aiCall";
...
const EvaluateResume = () => {
  ...
  useEffect(() => {
    const storedData = localStorage.getItem("jobAnalysis");
    const storedDataJson = storedData ? JSON.parse(storedData) : null;

    if (!storedDataJson) {
      console.error("No stored job analysis data found.");
      setLoading(false);
      return;
    }
    ...
    const fetchData = async () => {
      try {
        const result = await jobAnalysis(storedDataJson);

        if (!result) {
          console.error("Invalid response from jobAnalysis function");
          return;
        }

        setAnalysisData(result);
        calculateScores(result.analysis);
        setGrammarScore(result.grammar?.score || 0);
        setAtsScore(result.atsCompatibility?.score || 0);
        determineSuitability(result.overallMatch);
  ...
  const determineSuitability = (overallScore) => {
    if (overallScore < 50) {
      setSuitabilityMessage(
        "Your resume has significant room for improvement. Consider enhancing key skills and experiences relevant to the job."
      );
    } else if (overallScore >= 50 && overallScore < 80) {
      setSuitabilityMessage(
        "Your resume is on the right track but could benefit from targeted refinements. Improve weak areas to increase your chances."
      );
    } else {
      setSuitabilityMessage(
        "Your resume is strong and aligns well with the job description. Continue refining minor aspects for optimal results."
      );
    }
  };
  ...
  const barChartData = {
    labels:
      analysisData?.analysis?.map((fieldData) => fieldData.fieldName) || [],
    datasets: [
      {
        label: "Field Scores",
        data: fieldScores,
      ...
      <div className="bg-gray-900 p-6 rounded-lg w-3/4 max-w-4xl">
        ...
        {loading ? (
          ...
          <div>
            ...
            {improvements && (
              <div className="bg-green-200 p-6 rounded-lg shadow-md mt-6">
                <h3 className="text-xl font-semibold text-gray-800">
                  Overall Improvements
                </h3>
                <p className="text-gray-700 mt-2">{improvements}</p>
              </div>
            )}
          </div>
        )}
